...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Guilherme\n' // Nama kamu
            + 'ORG:Pepy;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5519999897566:+55 1999989-7566\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '/','*','.'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Digitalize o código qr  ))
	})
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD PEPY BOT*',
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmão. Você não está registrado como amigo de Pepy. Registre-se para fazer amizade com o bot Pepy por meio, \n\nCommand : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar Pepy|14\n\n──「 Pepy BOT 」──`,
			...
			const premium = ["5519999897566@s.whatsapp.net","559784130980@s.whatsapp.net","557599941062@s.whatsapp.net","558598669655@s.whatsapp.net","5511949051934@s.whatsapp.net","554792091566@s.whatsapp.net","558699541889@s.whatsapp.net","559294313229@s.whatsapp.net","554298653614@s.whatsapp.net"]
...
//@nabutoYT					
    ...
    break
            ...
            case 'rapL':
				...
				case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (args.length < 1) return reply(`Parâmetros Incorretos\nCommand : ${prefix}daftar nome|umur\nContoh : ${prefix}daftar Pepy|14`)
					var reg = body.slice(8)
					var jeneng = reg.split("|")[0];
					var umure = reg.split("|")[1];
						user.push(sender)
						fs.writeFileSync('./src/user.json', JSON.stringify(user))
						client.sendMessage(from, `\`\`\`O registro foi feito com sucesso  SN: TM08GK8HEUZBEHDO\`\`\`\n\n\`\`\`Sobre ${date} ${time}\`\`\`\n\`\`\`[Nome]: ${jeneng}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Era]: ${umure}\`\`\`\n\`\`\`Para usar um bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefix}help\`\`\`\n\`\`\`\nTotal de usuários ${user.length}\`\`\``, text, {quoted: mek})
					break
				case 'hunti':
					reply(mess.wait)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO Pepy BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @PEPY domina vida
          ...
          reply('Espere..')
			    ...
			    case 'dono':
                    ...
                    client.sendMessage(from, 'Meu dono é esse gasoso aí tal de "Guilherme"🙄, chama ele aí ele diz que é "Shitpost"',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT PEPY ✨* 」\n*│+ wa.me/5519999897566*╰──────*「 *PEPY* 」*────*\n\n*_SE QUER SER ADMIN DO BOT PEPY_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do PEPY:*\n\n https://youtube.com/channel/UCZEtf9AlsC2zsJQwrfW-44w'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* PEPY (não tenho canal no youtube, mais segue la no insta :) \n*INSTAGRAM:* https://instagram.com/hoshigaki_guilherme?igshid=10h9nmkvo6oy1 \n*WPP:* wa.me/+5519999897566\n*\n\nEspero que tenham gostado do bot️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
